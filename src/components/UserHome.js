...
export default function UserHome() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:3002/${userid}/available-books`)
...
const handleClickToOpen = (rowdata) => {
  ...
  const createExchReq = (rowdata) => {
    ...
    axios.post(`http://localhost:3002/${userid}/create-exchange-request`, {
      lender_id: rowdata.lender_id,
      book_id: rowdata.id,
    })
    .then(function (response) {
  ...
  return (
    ...
    <MaterialTable
      ...
      actions={[
        {
          icon:()=> {
            return <RuleFolderIcon style={{width:'18px', height:'18px'}} />
          },
          tooltip: 'Create exchange request',
          onClick: (event,rowData) => {
            alert(JSON.stringify(rowData))
            createExchReq(rowData)
          },
          isFreeAction: false
          }
      ]}
